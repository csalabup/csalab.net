...
class UpdateResizeImage extends Job implements SelfHandling, ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($id, $path, $bucket, $original_url)
    {
        $this->id = $id;
        $this->path = $path;
        $this->bucket = $bucket;
    ...
    public function handle()
    {
        ...
        $thumb_file_to_delete = $original->thumb_key;
        ...
        $original->thumb_url = 'https://s3.amazonaws.com/junecity.com/'.$path. '/' .$updated_thumb_file_name_60X60;

        $original->thumb_key = $path. '/' .$updated_thumb_file_name_60X60;
